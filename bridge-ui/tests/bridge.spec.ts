...
const openConnectedApp = async (page: Page, options: { rejectDeposit?: boolean; rejectSwitch?: boolean } = {}) => {
  ...
  if (!options.rejectSwitch) {
    ...
    await page.getByRole("button", { name: /Connect Zeko wallet/ }).click()
    await page.getByRole("button", { name: /Auro Wallet/ }).click()
    await expect(page.getByRole("button", { name: "Zeko wallet B62qpu…aeFF" })).toBeVisible({ timeout: 20_000 })
  }
}
...
test("builds the withdrawal review route with the Zeko Testnet wallet", async ({ page }) => {
  await openConnectedApp(page)
  ...
  await expect(page.getByText("Zeko wallet · Zeko Testnet")).toBeVisible()
  await expect(page.getByRole("button", { name: "Confirm in Zeko wallet" })).toBeVisible()
})
...
test("persists and reconnects the selected Zeko wallet provider", async ({ page }) => {
  await openConnectedApp(page)
  ...
  await expect(page.getByRole("button", { name: /Connect Zeko wallet/i })).toBeVisible()
  ...
  await page.getByRole("button", { name: /Connect Zeko wallet/i }).click()
