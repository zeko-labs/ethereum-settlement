...
export const ReviewView = ({ direction, amount, recipient, asset, config, busy, onBack, onConfirm }: {
  direction: Direction
  amount: string
  recipient: string
  asset: BridgeAsset
  ...
  return (
    <section className="review-view" data-screen-label="Review transfer">
      <div className="review-top"><div className="review-title"><h2>Review {direction}</h2><p>Confirm the route and recipient before opening your wallet.</p></div><div className="amount-lockup"><strong>{amount} {asset.symbol}</strong><span>{asset.kind === "native" ? "Native asset" : "Canonical ERC-20"}</span></div></div>
