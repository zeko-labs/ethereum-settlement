...
export const CompleteView = ({ direction, amount, symbol = "ETH", hash, url, onActivity, onNewTransfer }: {
  direction: Direction
  amount: string
  symbol?: string
  hash: string
  ...
  <section className="complete-view" data-screen-label="Transfer complete">
    ...
    <div><h2>{direction === "deposit" ? "Deposit finalized" : "Withdrawal claimed"}</h2><p>{amount} {symbol} completed its proof-bound route.</p></div>
    <div className="complete-amount">{amount} {symbol}</div>
