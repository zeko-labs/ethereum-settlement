...
export const BridgeForm = (props: Props) => {
  ...
  const assetKind = props.asset.kind === "native" ? "Native asset" : "Canonical ERC-20"
  return (
    <section className="bridge-form" data-screen-label="Bridge form">
      <div className="form-heading">
        <h2>{deposit ? "Deposit to Zeko" : "Withdraw to Ethereum"}</h2>
        <div className="route-kicker"><strong>{props.asset.symbol}</strong><span>·</span><span>{assetKind} · Verified settlement route</span></div>
      </div>
      <label className="asset-selector"><span>Asset</span><select aria-label="Bridge asset" value={props.asset.id} disabled={props.assetsLoading} onChange={(event) => props.onAssetChange(event.target.value)}>{props.assets.map((asset) => <option key={asset.id} value={asset.id}>{asset.symbol} · {asset.name}</option>)}</select>{props.assetsLoading && <small>Verifying registry…</small>}</label>
      <div className="transfer-surface">
        <div className="transfer-panel">
          <div className="amount-side">
            <div className="field-topline"><span>You send</span><span>{sourceBalance === undefined ? "Balance unavailable" : `Balance ${sourceBalance} ${props.asset.symbol}`}</span></div>
            <input className="amount-input" aria-label={props.asset.kind === "native" ? "Amount of native ETH to bridge" : `Amount of ${props.asset.symbol} to bridge`} inputMode="decimal" placeholder="0.00" value={props.amount} onChange={(event) => props.onAmountChange(normalizeAmountInput(event.target.value, props.asset.zekoDecimals))} />
            <span className="fiat-value">{assetKind} · maximum {props.asset.zekoDecimals} decimal places</span>
          </div>
        ...
        <div className="transfer-panel">
          ...
          <div className="network-side">
            <div className="network-pill"><NetworkIcon network={destination} /><span className="network-pill-copy"><span className="network-name">{deposit ? "Zeko Testnet" : ethereum}</span><span className="network-type">{deposit ? "Execution network" : "Settlement & custody"}</span></span><span className="token-label">{props.asset.symbol}</span></div>
