...
4. Deploy the immutable `ZekoAssetRegistry` module, then deploy
   `ZekoSettlement` and `EthereumZekoBridge` behind `ERC1967Proxy`. Initialize
   the full eight outer-state fields, action state/length, genesis timestamp,
   slot duration, fork slot, program vkey and SHA-256 PoC VK identifier.
...
- This live acceptance run remains native-only. Deposit cancellation/timeouts,
  proof fees, and registry-backed ERC-20 operation are outside that Sepolia
  acceptance boundary; the implemented local ERC-20 checkpoint is documented
  in [`docs/content/status.md`](docs/content/status.md). Legacy entry points
