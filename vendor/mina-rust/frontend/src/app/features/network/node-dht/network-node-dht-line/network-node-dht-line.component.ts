...
export class NetworkNodeDhtLineComponent
  ...
  implements AfterViewInit
{
  ...
  private listenToElWidthChange(): void {
    ...
    fromEvent(this.line.nativeElement, widthChange)
      .pipe(
        filter(() => !!this.thisKey),
        filter(
          () => this.lastKnownWidth !== this.line.nativeElement.offsetWidth,
  ...
  private listenToNodeDhtPeers(): void {
    this.select(
      selectNetworkNodeDhtKeyPeersBucketsOpenSidePanel,
      ([key, peers, buckets, openSidePanel]: [
        string,
        NetworkNodeDhtPeer[],
        NetworkNodeDhtBucket[],
        boolean,
      ]) => {
        ...
        if (
          this.thisKey &&
          (this.peers.length === 0 ||
            this.peers.some((p, i) => p.hexDistance !== peers[i].hexDistance))
        ) {
          this.peers = peers;
  ...
  private calculateLeftBasedOnParentContainer(): void {
    ...
    this.points.forEach(
      point => (point.left = (point.originalLeft / 100) * max),
    );
  }
  ...
  private calculate(): void {
    ...
    for (const bucket of buckets.slice().reverse()) {
      ...
      const left_percent =
        (Number(this_bucket_key_int) / Number(max_keyspace_int)) * 100;
  ...
  private getMaxOfHex(buckets: NetworkNodeDhtBucket[]): string {
    return buckets.reduce(
      (maxHex, bucket) =>
        maxHex > bucket.bucketMaxHex ? maxHex : bucket.bucketMaxHex,
      '0',
    );
  }
  ...
  selectPeer(point: DhtPoint): void {
    this.dispatch(
      ...
      this.peers.find(p => p.peerId === point.peerId),
    );
  }
}
