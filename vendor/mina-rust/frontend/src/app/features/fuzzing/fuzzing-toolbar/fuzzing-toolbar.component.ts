...
export class FuzzingToolbarComponent extends StoreDispatcher implements OnInit {
  ...
  private initForm(): void {
    ...
    this.formGroup
      .get('search')
      .valueChanges.pipe(untilDestroyed(this), debounceTime(150))
  ...
  private listenToFiles(): void {
    this.select(selectFuzzingFiles, (files: FuzzingFile[]) => {
      this.overallCoverage =
        files.reduce(
          (acc: number, file: FuzzingFile) => acc + file.coverage,
          0,
        ) / files.length;
      this.coverageColor =
        this.overallCoverage > 80
          ? 'success'
          : this.overallCoverage > 50
            ? 'aware'
            : 'warn';
      this.detect();
    });
