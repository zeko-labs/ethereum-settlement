...
const select = <T>(
  selector: (state: BlockProductionOverviewState) => T,
): MemoizedSelector<MinaState, T> =>
  createSelector(BlockProductionSelectors.overview, selector);

const epochs = select(
  (state: BlockProductionOverviewState): BlockProductionOverviewEpoch[] =>
    state.epochs,
);
const activeEpoch = select(
  (state: BlockProductionOverviewState): BlockProductionOverviewEpoch =>
    state.activeEpoch,
);
const allTimeStats = select(
  (state: BlockProductionOverviewState): BlockProductionOverviewAllStats =>
    state.allTimeStats,
);
const filters = select(
  (state: BlockProductionOverviewState): BlockProductionOverviewFilters =>
    state.filters,
);
const scale = select(
  (state: BlockProductionOverviewState): 'linear' | 'adaptive' => state.scale,
);
const loading = select((state: BlockProductionOverviewState) => ({
  isLoading: state.isLoading,
  isCalculatingVRF: state.isCalculatingVRF,
}));
const activeSlot = select(
  (
    state: BlockProductionOverviewState,
  ): BlockProductionOverviewSlot | undefined => state.activeSlot,
);
