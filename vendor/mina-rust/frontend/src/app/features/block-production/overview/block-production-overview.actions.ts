...
const type = <T extends string>(type: T) =>
  createType(BLOCK_PRODUCTION_PREFIX, 'Overview', type);

const init = createAction(type('Init'));
const close = createAction(type('Close'));
const getEpochs = createAction(
  type('Get Epochs'),
  props<{ epochNumber: number }>(),
);
const getEpochsSuccess = createAction(
  type('Get Epochs Success'),
  props<{ epochs: BlockProductionOverviewEpoch[] }>(),
);
const getSlots = createAction(
  type('Get Slots'),
  props<{ epochNumber: number }>(),
);
const getSlotsSuccess = createAction(
  type('Get Slots Success'),
  props<{ slots: BlockProductionOverviewSlot[] }>(),
);
const getEpochDetails = createAction(
  type('Get Epoch Details'),
  props<{ epochNumber: number }>(),
);
const getEpochDetailsSuccess = createAction(
  type('Get Epoch Details Success'),
  props<{
    details: BlockProductionOverviewEpochDetails;
  }>(),
);
const getRewardsStats = createAction(type('Get Rewards Stats'));
const getRewardsStatsSuccess = createAction(
  type('Get Rewards Stats Success'),
  props<{
...
const changeFilters = createAction(
  type('Change Filters'),
  props<{ filters: BlockProductionOverviewFilters }>(),
);
const changeScale = createAction(
  type('Change Scale'),
  props<{ scale: 'linear' | 'adaptive' }>(),
);
