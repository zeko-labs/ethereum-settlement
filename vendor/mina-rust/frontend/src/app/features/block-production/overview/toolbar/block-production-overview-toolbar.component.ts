...
export class BlockProductionOverviewToolbarComponent
  ...
  implements OnInit
{
    ...
    this.select(
      ...
      (activeEpoch: BlockProductionOverviewEpoch) => {
        ...
        if (activeEpoch?.slots) {
          this.totalCanonical = activeEpoch.slots.filter(
            s => s.canonical,
          ).length;
          ...
          this.totalFuture = activeEpoch.slots.filter(
            s => s.futureRights,
          ).length;
        } else {
  ...
  changeFilter(
    filter: keyof BlockProductionOverviewFilters,
    value: boolean,
  ): void {
    this.dispatch2(
      BlockProductionOverviewActions.changeFilters({
  ...
  changeActiveEpoch(newEpoch: number): void {
    this.dispatch2(
      BlockProductionOverviewActions.getEpochDetails({ epochNumber: newEpoch }),
    );
  }
