...
const select = <T>(
  selector: (state: BlockProductionState) => T,
...
const overview = select(
  (state: BlockProductionState): BlockProductionOverviewState =>
    state[BLOCK_PRODUCTION_OVERVIEW_KEY],
);
const wonSlots = select(
  (state: BlockProductionState): BlockProductionWonSlotsState =>
    state[BLOCK_PRODUCTION_WON_SLOTS_KEY],
);
