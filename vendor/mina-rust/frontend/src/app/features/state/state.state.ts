...
): MemoizedSelector<MinaState, T> => createSelector(selectStateState, selector);

export const selectStateState = createFeatureSelector<StateState>('state');
export const selectStateActionsState = select(
  (state: StateState): StateActionsState => state.actions,
);
