...
import {
  ...
  selectStateActionsOpenSidePanel,
...
export class StateActionsGraphListComponent
  extends StoreDispatcher
  implements OnInit
{
  readonly X_STEPS: string[] = [
    '0μs',
    '1μs',
    '10μs',
    '50μs',
  ...
  readonly trackGroup = (index: number, group: StateActionGroup): string =>
    group.groupName + group.totalTime + group.meanTime + group.count;
  readonly trackAction = (
    index: number,
    action: StateActionGroupAction,
  ): string => action.title;
