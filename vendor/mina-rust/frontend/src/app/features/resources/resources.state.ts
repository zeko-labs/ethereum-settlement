...
const select = <T>(
  selector: (state: ResourcesState) => T,
): MemoizedSelector<MinaState, T> =>
  createSelector(selectResourcesState, selector);

export const selectResourcesState =
  createFeatureSelector<ResourcesState>('resources');
export const selectMemoryResourcesState = select(
  (state: ResourcesState): MemoryResourcesState => state.memory,
);
