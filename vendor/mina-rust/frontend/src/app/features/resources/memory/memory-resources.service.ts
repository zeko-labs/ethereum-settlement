...
export class MemoryResourcesService {
  ...
  ): Observable<MemoryResource> {
    this.id = 0;
    return this.rust
      .getMemProfiler<MemoryResourceTree>(
        `/v1/tree?threshold=${threshold}&reverse=${reversed}`,
      )
      .pipe(
        map((response: MemoryResourceTree) =>
          this.mapMemoryResponse(response, threshold),
        ),
      );
  }

  private mapMemoryResponse(
    response: MemoryResourceTree,
    threshold: number,
  ): MemoryResource {
    return {
  ...
  private build(
    ...
    return children.sort(
      (c1: MemoryResource, c2: MemoryResource) => c2.value - c1.value,
    );
  }

  private getFrameName(
    name: MemoryResourceTree['name'],
    threshold: number,
  ): MemoryResourceName {
    if (typeof name === 'string') {
      return {
        executableName:
          name === 'underThreshold' ? `below ${round(threshold)} KB` : name,
        functionName: null,
      };
    }
