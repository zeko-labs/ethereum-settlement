...
impl MontConfig<4> for FqConfig {
    ...
    fn mul_assign(a: &mut Fp256<MontBackend<Self, 4>>, b: &Fp256<MontBackend<Self, 4>>) {
        ...
        let mut tmp = [0u64; 4];
        sp1_zkvm::syscalls::sys_bigint(&mut tmp, 0, &(a.0).0, &(b.0).0, &Self::MODULUS.0);
        sp1_zkvm::syscalls::sys_bigint(&mut (a.0).0, 0, &tmp, &R_INV, &Self::MODULUS.0);
    }
    ...
    fn sum_of_products<const M: usize>(
        ...
        let mut acc = Fp256::<MontBackend<Self, 4>>::new_unchecked(BigInt::new([0u64; 4]));
        for i in 0..M {
            let mut tmp = [0u64; 4];
            sp1_zkvm::syscalls::sys_bigint(&mut tmp, 0, &(a[i].0).0, &(b[i].0).0, &Self::MODULUS.0);
            acc += Fp256::<MontBackend<Self, 4>>::new_unchecked(BigInt::new(tmp));
        }
        let mut result = [0u64; 4];
        sp1_zkvm::syscalls::sys_bigint(&mut result, 0, &(acc.0).0, &R_INV, &Self::MODULUS.0);
