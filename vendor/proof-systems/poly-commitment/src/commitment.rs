...
impl<C: AffineRepr> PolyComm<C> {
    ...
    pub fn multi_scalar_mul(com: &[&PolyComm<C>], elm: &[C::ScalarField]) -> Self {
        ...
        #[cfg(target_os = "zkvm")]
        {
            let chunks_len = Iterator::max(com.iter().map(|c| c.chunks.len())).unwrap();
            let chunks = (0..chunks_len)
                .map(|chunk| {
                    let mut acc = C::Group::zero();
                    for (poly_comm, scalar) in com.iter().zip(elm) {
                        if let Some(point) = poly_comm.chunks.get(chunk) {
